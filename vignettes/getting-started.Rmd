...
2. **Matching layerId/rowId**: Components must use the shared ID for selection. For example, `layerId` in `leaflet`, row names in `DT`, and `customdata` or `key` in `plotly`.
...
- **Chart row click**: Dim all other points, highlight selected point
- **Consistent behavior**: All interactions produce the same visual result
